...
import { surveyListingPost, surveyDelete } from "../api";
...
const QuestionairesListing = () => {
    ...
    const getSurveys = async () => {
        setLoading(true)
        setSurveys([]);
        let result = await surveyListingPost({
            // pageSize: pageSize, 
            // lastKey: pages[pageIndex], 
            status: status
        })
        console.log("surveys result", result);
        if(result.Success) {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">Survey Questionaires</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/survey/questionaires/new")}>+ NEW SURVEY QUESTIONAIRE</button>
                    </div>
                        ...
                        <tbody>
                            {
                                ...
                                surveys
                                        .map(
                                            (x, index) => (
                                                <tr key={`survey_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    {/* <td>
                                                        <Checkbox
                                                            key={x.AssetId}
                                                            type="checkbox"
...
export default QuestionairesListing