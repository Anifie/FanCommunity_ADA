...
import { artworkListingPost, enumGet, artworkDelete, artworkMintPost } from "../api";
...
const ArtworkListing = () => {
    ...
    const getArtworks = async () => {
        setLoading(true)
        setArtworks([]);
        let result = await artworkListingPost({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            ...
            if(result.Data.artworks.length > 0 && result.Data.nextToken) {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">Artwork</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/artwork/create")}>+ CREATE NEW</button>
                    </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`artwork_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        {x.TwoDURL && <a href={x.TwoDURL}><img src={window.location.href.includes("https://") ? x.TwoDURL.replace("http://", "https://") : x.TwoDURL.replace("https://", "http://")} className="max-w-[100px]" /></a>}
                                                    </td>
                                                    <td>
                                                        {x.Name}
                                                        <br/>
                                                        {x.Description}
                                                        {/* {x.TwoDURL_2 && <a href={x.TwoDURL_2}><img src={x.TwoDURL_2} className="max-w-[100px]" /></a>} */}
                                                    </td>
                                                    {/* <td>
                                                        {x.TwoDURL_3 && <a href={x.TwoDURL_3}><img src={x.TwoDURL_3} className="max-w-[100px]" /></a>}
                                                    </td> */}
                                                    {/* <td>
                                                        {x.ThreeDURL && <a href={x.ThreeDURL}>Download</a>}
                                                    </td> */}
                                                    {/* <td>
                                                        {x.ArtworkType}
                                                    </td> */}
                                                    <td>
                                                        ...
                                                        {x.SubCategory && x.SubCategory != 'undefined' && <><br/>{x.SubCategory}</>}
                                                    </td>
                                                    <td>
                                                        {x.ArtworkId && <>Artwork Id: {x.ArtworkId}</>}
                                                        {x.MemberId && <><br/>Member Id: {x.MemberId}</>}
                                                    </td>
                                                    {/* <td>
                                                        { 
                                                            x.Components &&
                                                            <Tooltip tooltipText={JSON.stringify(x.Components)}>
                                                                <span>{JSON.stringify(x.Components).substring(0, 19) + (JSON.stringify(x.Components).length > 20 ? "..." : "")}</span>
                                                            </Tooltip>
                                                        }
                                                    </td> */}
                                                    {/* <td>
                                                        {x.NameEN && <span>{x.NameEN} : {x.ValueEN}</span>}
                                                        {x.NameJP && <span><br/>{x.NameJP} : {x.ValueJP}</span>}
                                                    </td> */}
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => router.push("/artwork/edit/" + x.ArtworkId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => mintArtwork(x.ArtworkId, x.Category)}>Mint</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteArtwork(x.ArtworkId)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
