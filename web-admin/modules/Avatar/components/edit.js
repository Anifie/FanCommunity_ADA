...
import { avatarPut, avatarGet } from "../api";
...
const AvatarEdit = () => {
    ...
    useEffect(() => {
        if(avatarCategories && avatarCategories.length > 0 && avatarSubCategories && avatarSubCategories.length > 0 && avatarItems && avatarItems.length > 0 && avatarId != undefined) {
            getAvatar(avatarId)
        }    
    }, [avatarCategories, avatarSubCategories, avatarItems, avatarId])
    
    const getAvatar = async (avatarId) =>  {
        let result = await avatarGet({avatarId: avatarId});
        if(result.Success) {
            setAvatar(result.Data.Avatar);

            setValue("playerid", result.Data.Avatar.MemberId)
            setValue("itemidhead", result.Data.Avatar.HeadItemId)
            setValue("itemidbody", result.Data.Avatar.BodyItemId)
            setValue("status", result.Data.Avatar.Status)
            
            const avatarHeadItem = avatarItems.find(x => x.AvatarItemId == result.Data.Avatar.HeadItemId)
            const category = avatarCategories.find(x => x.AvatarCategoryId === avatarHeadItem.AvatarCategoryId)
            setValue("avatarcategory", category.AvatarCategoryId + " : " + category.Name)

            for (let j = 0; j < result.Data.Avatar.ItemIds.length; j++) {
                const clothItemId = result.Data.Avatar.ItemIds[j];
                console.log("clothItemId", clothItemId);
                for (let i = 0; i < avatarItems.length; i++) {
                    const avatarItem = avatarItems[i];
                    console.log("avatarItem", avatarItem);

                    if(avatarItem.AvatarItemId === clothItemId) {
                        
                        const subCategory = avatarSubCategories.find(x => x.AvatarSubCategoryId === avatarItem.AvatarSubCategoryId)

                        switch (subCategory.ClassName) {
                            case "TOP":
                                setValue("itemidtop", clothItemId)
                                break;
                        
                            case "BOTTOM":
                                setValue("itemidbottom", clothItemId)
                                break;

                            case "HAIR":
                                setValue("itemidhair", clothItemId)
                                break;

                            case "SHOES":
                                setValue("itemidshoes", clothItemId)
                                break;
                                
                            case "GLASSES":
                                setValue("itemidglasses", clothItemId)
                                break;
                                
                            default:
                                throw new Error("Unexpected class name: " + subCategory.ClassName)
                                break;
                        }
                        break;
                    }
            ...
            showFailed("Failed to load avatar")
        }
    }
    ...
    const onSubmit = async (data) => {
        ...
        if(result.Success) {
            ...
            showFailed("Avatar update failed with message: " + result.Message)
    }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">EDIT AVATAR</h2>                        
                </div>
                <div className="p-4 w-full">
                            ...
                            <div className="flex flex-col">
                                <div className="flex flex-col col-span-2 mt-2">
                                    ...
                                    <input type="text" 
                                        ...
                                        {...register("avatarcategory", {required: false})} />
                                </div>
                                <div className="flex flex-col col-span-2 mt-2">
                                ...
                                {/* <div className="flex flex-col col-span-2 mt-2">
                                    ...
                                    <textarea className="textarea textarea-bordered max-w-lg" cols="100" 
                                        {...register("itemids", {required: false})} />
                                </div> */}
                                <div className="flex flex-col col-span-2 mt-2">
                                    <label>Status</label>
                    ...
                    <br/>
                    <button className="mt-5 btn btn-primary btn-sm" type="submit">Update Avatar</button>
