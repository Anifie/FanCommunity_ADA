...
import { roleMemberListingPost, roleMemberDelete } from "../api";
...
const RoleMemberListing = () => {
    ...
    const getRoleMembers = async () => {
        setLoading(true)
        setRoleMembers([]);
        let result = await roleMemberListingPost({
            // pageSize: pageSize, 
        ...
        console.log("role members result", result);
        if(result.Success) {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">ROLE MEMBER</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/role/member/assign")}>+ ASSIGN ROLE TO MEMBER</button>
                    </div>
...
export default RoleMemberListing