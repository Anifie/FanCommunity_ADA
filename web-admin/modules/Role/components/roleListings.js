...
import { roleListingGet, roleDelete } from "../api";
...
const RoleListing = () => {
    ...
    const getRoles = async () => {
        setLoading(true)
        setRoles([]);
        let result = await roleListingGet({
            // pageSize: pageSize, 
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">ROLE</h2>   
            </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`role_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    {/* <td>
                                                        {x.Permissions ? JSON.stringify(x.Permissions) : ''}
                                                    </td> */}
                                                    <td>
