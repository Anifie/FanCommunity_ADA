...
import { campaignCodeListingPost } from "../api";
...
const CampaignCodeListing = () => {
    ...
    const getCampaignCodes = async () => {
        setLoading(true)
        setCampaignCodes([]);
        let result = await campaignCodeListingPost({
            // pageSize: pageSize, 
    ...
    //     let result = await whitelistDelete({
    //         whitelistId: idArr[0],
    //         memberId: idArr[1]
    //     })
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">Membership NFT Campaign Code</h2>   
            </div>
...
export default CampaignCodeListing