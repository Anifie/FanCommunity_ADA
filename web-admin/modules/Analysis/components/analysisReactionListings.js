...
import { batchReactionListingPost } from "../api";
...
const AnalysisMessageListing = () => {
    ...
    const getBatches = async () => {
        setLoading(true)
        setMessages([]);
        let result = await batchReactionListingPost({
            pageSize: pageSize, 
            nextToken: pages[pageIndex], 
            // createdDate: process.env.REACTION_START_DATE
            // status: status, 
            // batchId: batchId
        })
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">DISCORD CHAT REACTIONS</h2>   
            </div>
