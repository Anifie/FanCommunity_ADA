...
const AdministratorCreate = () => {
    ...
    const [adminUserName, setAdminUserName] = useState({value: "", dirty: false})
    ...
    const handleFormSubmit = async (event) => {
        ...
        let result = await administratorPost({
                                            token: localStorage.getItem("tokyodome_admin_access_token"), 
                                            userName: adminUserName.value,
                                            password: password.value
                                        })

        console.log("createAdministrator result", result);
        if(result.Success) {
            ...
            showFailed("Failed to create administrator")
        }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleFormSubmit}>
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">ADD NEW ADMINISTRATOR</h2>                        
                </div>
                <div className="p-4 w-full">
                    ...
                    <input type="email" 
                        ...
                        onChange={(e) => setAdminUserName({value: e.target.value, dirty: true})} 
                        placeholder="Email" 
                        className="input input-bordered w-full max-w-lg" />
                    {adminUserName.dirty && adminUserName.value == "" && <p className="text-red-500">Email is required</p>}
