...
import { stickerIdListingGet } from "../api";
...
const StickerIdListing = () => {
    ...
    const getStickerIds = async () => {
        setLoading(true)
        setStickerIds([]);
        let result = await stickerIdListingGet({
            // pageSize: pageSize, 
        ...
        console.log("stickerIds result", result);
        if(result.Success) {
            setStickerIds(result.Data)
    ...
    //     mdConfirm.current.show("Confirm", "Confirm Delete Role '" + roleName + "' ?", "Delete", confirmDeleteRole, roleName)
    // }

    // const confirmDeleteRole = async (roleName) => {
    //     mdLoading.current.show("Deleting..")
    //     let result = await roleDelete({roleName: roleName})
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">STICKER ID</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        {/* <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/role/create")}>+ CREATE NEW</button> */}
                    </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`role_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        <img src={x.BarcodeImageURL} className="h-20"/>
                                                    </td>
                                                    <td>
                                                        { x.IsUsed ? 'USED' : ''}
                                                    </td>
                                                    ...
                                                    {/* <td>
                                                        <div className="dropdown dropdown-right">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                <a onClick={() => router.push("/role/edit/" + x.Name)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteRole(x.Name)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td> */}
...
export default StickerIdListing