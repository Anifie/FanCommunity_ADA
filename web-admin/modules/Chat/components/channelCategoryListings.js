...
import { chatChannelCategoryDelete, chatChannelCategoryListingGet } from "../api";
...
const ChatListing = () => {
    ...
    const [chatChannelCategories, setChatChannelCategories] = useState([])
    ...
    const getChatChannelCategories = async () => {
        setLoading(true)
        setChatChannelCategories([]);
        let result = await chatChannelCategoryListingGet({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setChatChannelCategories(result.Data)
    ...
    const deleteChatChannelCategory = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Chat Channel Category with Id '" + id + "' ?", "Delete", confirmDeleteChatChannelCategory, id)
    }
    ...
    const confirmDeleteChatChannelCategory = async (id) => {
        mdLoading.current.show("Deleting..")
        let result = await chatChannelCategoryDelete({
            chatChannelCategoryId: id
        })
        console.log("delete result", result);
        if(result?.Success){
            showSuccess("Chat Category Deleted")
            await getChatChannelCategories()
        }
        else {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - CHANNEL CATEGORY</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/chat/channel-category/create")}>+ CREATE NEW</button>
                    </div>
                    ...
                    <table className={`table table-compact w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#242933]'}`}>
                        ...
                        <tbody>
                            {
                                chatChannelCategories && chatChannelCategories.length > 0
                                ?
                                chatChannelCategories
                                        .map(
                                            (x, index) => (
                                                <tr key={`chatrooms_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                        {x.ChatChannelCategoryId}
                                                    </td>
                                                    <td>
                                                        {x.Name}
                                                    </td>
                                                    ...
                                                    <td>
                                                        {
                                                            x.AccessType == 'CATEGORY_PRIVATE' && x.RolesAndMembers &&
                                                            <>
                                                                Roles:<br/>
                                                                { x.RolesAndMembers.roles }
                                                                <br/>
                                                                <br/>
                                                                Members:<br/>
                                                                { x.RolesAndMembers.members }
                                                            </>
                                                        }
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => router.push("/chat/channel-category/edit/" + x.ChatChannelCategoryId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteChatChannelCategory(x.ChatChannelCategoryId)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
