...
import { chatChannelCategoryPost, enumGet } from "../api";

// const toolbarPlugin = createToolbarPlugin();
  
const ChatCategoryNew = () => {
    ...
    const onSubmit = async (data) => {
        ...
        let result = await chatChannelCategoryPost({
        ...
        console.log("createChatChannelCategory result", result)
        
        if(result.Success) {

            mdLoading.current.close()
            showSuccess("Chat Channel Category created successfully")

            router.push("/chat/channel-category")
        }            
        else {
            console.error(result.Message)
            showFailed("Failed to create Chat Channel Category")
        }
    }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">ADD NEW CHAT CHANNEL CATEGORY</h2>                        
                </div>
                        ...
                        <div className="grid sm:grid-cols-1 grid-cols-2 gap-2">
                            ...
                            <div className="flex flex-col w-1/2">
                                ...
                                <textarea {...register("categorypermissions", {required: false})} rows={10}>
                                    {JSON.stringify(sampleCategoryPermission, null, 2)}
                                </textarea>
                            </div>
                        </div>
                    }
...
export default ChatCategoryNew