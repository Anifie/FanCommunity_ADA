...
import { threadMessagePost, chatChannelMessageListingGet, enumGet } from "../api";
  
const ThreadMessageNew = () => {
    ...
    const onSubmit = async (data) => {
        ...
        let result = await threadMessagePost({
        ...
        console.log("post Thread Message result", result)
        
        if(result.Success) {

            mdLoading.current.close()
            showSuccess("Thread Message posted successfully")

            router.push("/chat/thread-message")
        }            
        else {
            console.error(result.Message)
            showFailed("Failed to post Thread Message")
        }
    }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">POST NEW THREAD MESSAGE</h2>                        
                </div>
                <div className="p-4 w-full">    
                    ...
                    <br/>
                    <div className="flex flex-col">
                        <label>Thread Id<span className="text-red-400"> *</span></label>
                        <input type="text" 
                                    placeholder="Thread Id" 
                                    className="input input-bordered max-w-lg"
                                    {...register("threadid", {required: false})} />
                        {errors.threadid?.type === 'required' && <p className="text-red-500">Thread Id is required</p>}
                    </div>
                    <br/>
...
export default ThreadMessageNew