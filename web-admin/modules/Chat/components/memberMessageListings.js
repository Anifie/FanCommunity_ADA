...
import { chatChannelDelete, memberMessageListingGet } from "../api";
...
const ChatChannelListing = () => {
    ...
    const getChatChannels = async () => {
        setLoading(true)
        setChatChannels([]);
        let result = await memberMessageListingGet({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setChatChannels(result.Data.messages)
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - MEMBER MESSAGE</h2>   
            </div>
