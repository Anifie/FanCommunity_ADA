...
import { chatChannelMessagePut, chatChannelMessageListingGet, enumGet } from "../api";
...
const ChatChannelMessageEdit = () => {
    ...
    const onSubmit = async (data) => {
        ...
        let result = await chatChannelMessagePut({
                                        messageId: messageId,
        ...
        console.log("edit ChatChannel Message result", result)
        
        if(result.Success) {

            mdLoading.current.close()
            showSuccess("Chat Channel Message updated successfully")

            router.push("/chat/channel-message")
        }            
        else {
            console.error(result.Message)
            showFailed("Failed to update Chat Channel Message")
        }
    }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">EDIT CHAT CHANNEL MESSAGE</h2>                        
                </div>
