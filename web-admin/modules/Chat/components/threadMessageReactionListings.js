...
import { reactThreadMessageDelete, threadMessageReactionListing } from "../api";
...
const ThreadMessageReactionListing = () => {

    //const {account} = useContext(Web3Context)

    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [threadReactions, setThreadMessageReactions] = useState([])
    ...
    const getMessageReactions = async () => {
        setLoading(true)
        setThreadMessageReactions([]);
        let result = await threadMessageReactionListing({
            pageSize: pageSize, 
        ...
        console.log("reactions result", result);
        if(result.Success) {
            setThreadMessageReactions(result.Data.reactions)
            setLastPageIndex(null);
            if(result.Data.reactions.length > 0 && result.Data.nextToken) {
    ...
    const deleteThreadMessageReaction = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Reaction with Id '" + id + "' ?", "Delete", confirmDeleteThreadMessageReaction, id)
    }
    ...
    const confirmDeleteThreadMessageReaction = async (id) => {
        mdLoading.current.show("Deleting..")
        let threadMsgId = id.split('_')[0];
        let emoId = id.split('_')[1];
        let reacterId = id.split('_')[2];
        let result = await reactThreadMessageDelete({
            threadMessageId: threadMsgId,
            emojiId: emoId,
            reactorId: reacterId
        })
        console.log("delete result", result);
        if(result?.Success){
            showSuccess("Message Reaction Deleted")
            ...
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    ...
                    <table className={`table table-compact w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#242933]'}`}>
                        ...
                        <tbody>
                            {
                                threadReactions && threadReactions.length > 0
                                ?
                                threadReactions
                                        .map(
                                            (x, index) => (
                                                <tr key={`reaction_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                        Chat Channel Id: {x.ChatChannelId}<br/>
                                                        Message Id: {x.MessageId}<br/>
                                                        Thread Message Id: {x.ThreadMessageId}
                                                    </td>
                                                    <td>
                                                        Member Id: {x.MemberId}
                                                    </td>
                                                    <td>
                                                        {
                                                            x.EmojiId
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => deleteThreadMessageReaction(`${x.ThreadMessageId}_${x.EmojiId}_${x.MemberId}`)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
...
export default ThreadMessageReactionListing