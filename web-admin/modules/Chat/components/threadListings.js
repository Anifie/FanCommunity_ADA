...
import { threadDelete, threadListingGet } from "../api";
...
const ThreadListing = () => {
    ...
    const getThreads = async () => {
        setLoading(true)
        setThreads([]);
        let result = await threadListingGet({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            ...
            if(result.Data.threads.length > 0 && result.Data.lastKey) {
    ...
    const deleteThread = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Thread with Id '" + id + "' ?", "Delete", confirmDeleteThread, id)
    }
    ...
    const confirmDeleteThread = async (id) => {
        mdLoading.current.show("Deleting..")
        let channelId = id.split('_')[0];
        let threadId = id.split('_')[1];
        let result = await threadDelete({
            threadId: threadId,
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - THREAD</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/chat/thread/create")}>+ CREATE NEW THREAD</button>
                    </div>
                        ...
                        <tbody>
                            {
                                ...
                                threads
                                        .map(
                                            (x, index) => (
                                                <tr key={`threads_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                        Thread Id : {x.ThreadId}
                                                        {
                                                            ...
                                                            <>
                                                            <br/>
                                                            Thread Name : {x.ThreadName}
                                                            </>
                                                        }
                                                    </td>
                                                    <td>
                                                        Channel ID : {x.ChatChannel.ChatChannelId} <br/>
                                                        Name : {x.ChatChannel.Name}<br/>
                                                        {
                                                            x.Message &&
                                                            <>
                                                            Message ID : {x.Message.MessageId} 
                                                            </>   
                                                        }
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        {x.AccessType}
                                                        {
                                                            ...
                                                            <>
                                                            <br/>
                                                            Member Ids : {x.ThreadMemberIds}
                                                            </>
                                                        }
                                                    </td>
                                                    <td>
                                                        Member ID : {x.Creator.MemberId} <br/>
                                                        Name : {x.Creator.DisplayName}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                {/* <li>
                                                                    <a onClick={() => router.push("/chat/channel/edit/" + x.ChatChannelId)}>Send Message in Thread</a>
                                                                </li> */}
                                                                <li>
                                                                    <a onClick={() => deleteThread(x.ChatChannel.ChatChannelId + "_" + x.ThreadId)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
