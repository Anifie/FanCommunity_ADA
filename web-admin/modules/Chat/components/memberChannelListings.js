...
import { memberChannelDelete, memberChannelListingGet } from "../api";
...
const MemberChannelListing = () => {
    ...
    const getMemberChannels = async () => {
        setLoading(true)
        setMemberChannels([]);
        let result = await memberChannelListingGet({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setMemberChannels(result.Data.memberChannels)
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - MEMBER CHANNEL</h2>   
            </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`chatrooms_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        Channel Id: {x.ChatChannel.ChatChannelId}<br/>
                                                        Channel Name: {x.ChatChannel.Name}
                                                    </td>
                                                    <td>
                                                        {x.MemberId}
                                                    </td>
                                                    <td>
                                                        { x.MemberLastReadMessageId}
                                                    </td>
                                                    <td>
                                                        {
                                                            x.MemberPermissions &&
...
export default MemberChannelListing