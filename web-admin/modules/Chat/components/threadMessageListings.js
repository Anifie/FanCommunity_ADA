...
import { threadMessageDelete, threadMessageListingPost } from "../api";
...
const ThreadMessageListing = () => {
    ...
    const getThreadMessages = async () => {
        setLoading(true)
        setThreadMessages([]);
        let result = await threadMessageListingPost({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setThreadMessages(result.Data.threadMessages)
            setLastPageIndex(null);
            if(result.Data.threadMessages.length > 0 && result.Data.nextToken) {
    ...
    const deleteThreadMessage = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Thread Message with Id '" + id + "' ?", "Delete", confirmDeleteThreadMessage, id)
    }
    ...
    const confirmDeleteThreadMessage = async (id) => {
        mdLoading.current.show("Deleting..")
        let result = await threadMessageDelete({
            threadMessageId: id
        })
        console.log("delete result", result);
        if(result?.Success){
            showSuccess("Thread Message Deleted")
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - THREAD MESSAGE</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/chat/thread-message/create")}>+ CREATE NEW</button>
                    </div>
                    <div className={`card w-full shadow- mb-4 ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                        <div className="card-body">
                            <div className="grid grid-cols-4 gap-2">
                                ...
                                <div className="flex flex-col">
                                    <label>Thread Message Id</label>
                                    <input type="text" 
                                        placeholder="Thread Message Id" 
                        ...
                        <tbody>
                            {
                                ...
                                threadMessages
                                        .map(
                                            (x, index) => (
                                                <tr key={`threadmessages_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                        ...
                                                        {x.ChatChannel.ChatChannelCategory && <>Category: {x.ChatChannel.ChatChannelCategory.Name}</>} <br/>
                                                        Thread Id : {x.ThreadId} <br/>
                                                    </td>
                                                    <td>
                                                        
                                                        Thread Message Id: {x.ThreadMessageId} <br/>
                                                        Thread Message: {typeof x.Message == 'object'  ? JSON.stringify(x.Message) : x.Message} <br/>
                                                        {
                                                            x.ActionOptions &&
                                                            <>
                                                    ...
                                                    <td>
                                                        {
                                                            x.ReplyThreadMessage &&
                                                            <>
                                                                Thread Message Id: {x.ReplyThreadMessage.ThreadMessageId} <br/>
                                                                Message: {x.ReplyThreadMessage.Message} <br/>
                                                                Member Id: {x.ReplyThreadMessage.Sender.MemberId}&nbsp;({x.ReplyThreadMessage.Sender.DisplayName})
                                                            </>
                                                        }
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        Pinned : { x.IsPinned === true ? 'Pinned' : ''} <br/>
                                                        Status : { x.Status} <br/>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                {/* <li>
                                                                    <a onClick={() => router.push("/chat/thread-message/edit/" + x.ThreadMessageId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteThreadMessage(x.ThreadMessageId)}>Delete</a>
                                                                </li> */}
                                                            </ul>
                                                        </div>
                                                    </td>
...
export default ThreadMessageListing