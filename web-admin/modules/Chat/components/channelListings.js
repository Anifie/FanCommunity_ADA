...
import { chatChannelDelete, chatChannelListingGet } from "../api";
...
const ChatChannelListing = () => {
    ...
    const getChatChannels = async () => {
        setLoading(true)
        setChatChannels([]);
        let result = await chatChannelListingGet({
            pageSize: pageSize, 
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - CHANNEL</h2>   
            </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`chatrooms_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        {
                                                            ...
                                                            <>
                                                                Category: {x.ChatChannelCategory.ChatChannelCategoryId}
                                                                <br/>
                                                            </>
                                                        }
                                                        Channel: {x.ChatChannelId}
                                                    </td>
                                                    <td>
                                                        {x.Name}
                                                    </td>
                                                    ...
                                                    <td>
                                                        {
                                                            x.AccessType == 'CHANNEL_PRIVATE' && x.RolesAndMembers &&
                                                            <>
                                                                Roles:<br/>
