...
import { chatChannelDelete, memberThreadMessageListingGet } from "../api";
...
const ChatChannelListing = () => {
    ...
    const getThreadMessages = async () => {
        setLoading(true)
        setThreadMessages([]);
        let result = await memberThreadMessageListingGet({
            pageSize: pageSize, 
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT THREAD - MEMBER MESSAGE</h2>   
            </div>
                        ...
                        <tbody>
                            {
                                ...
                                threadMessages
                                        .map(
                                            (x, index) => (
                                                <tr key={`threadmessage_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                        Channel Id : {x.ChatChannelId} <br/>
                                                        Message Id : {x.MessageId} <br/>
                                                        Thread Id : {x.ThreadId}
                                                    </td>
                                                    <td>
                                                        {x.ThreadMessageId}
                                                    </td>
                                                    <td>
                                                        { x.Sender.MemberId } ({x.Sender.DisplayName})
                                                    </td>
                                                    <td>
                                                        {
                                                           x.Receiver.MemberId
                                                        } ({x.Receiver.DisplayName})
                                                    </td>
                                                    <td>
                                                        {x.IsRead ? 'Seen' : ''}
                                                    </td>
...
export default ChatChannelListing