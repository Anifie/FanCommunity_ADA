...
import { memberChannelDelete, memberThreadListingGet } from "../api";
...
const MemberThreadListing = () => {
    ...
    const getMemberThreads = async () => {
        setLoading(true)
        setMemberThreads([]);
        let result = await memberThreadListingGet({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setMemberThreads(result.Data.memberThreads)
            setLastPageIndex(null);
            if(result.Data.memberThreads.length > 0 && result.Data.nextToken) {
    ...
    const deleteChatChannel = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Chat Channel with Id '" + id + "' ?", "Delete", confirmDeleteChatChannel, id)
    }
    ...
    const confirmDeleteChatChannel = async (id) => {
        mdLoading.current.show("Deleting..")
        let result = await chatChannelDelete({
            chatChannelId: id
        })
        console.log("delete result", result);
        if(result?.Success){
            showSuccess("Chat Channel Deleted")
            await getMemberThreads()
        }
        else {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - MEMBER THREAD</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    {/* <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/chat/channel/create")}>+ CREATE NEW</button>
                    </div> */}
                    ...
                    <table className={`table table-compact w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#242933]'}`}>
                        ...
                        <tbody>
                            {
                                memberThreads && memberThreads.length > 0
                                ?
                                memberThreads
                                        .map(
                                            (x, index) => (
                                                <tr key={`chatrooms_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        Thread Id: {x.Thread.ThreadId}<br/>
                                                        Thread Name: {x.Thread.THreadName}
                                                    </td>
                                                    <td>
                                                        {x.MemberId}
                                                    </td>
                                                    <td>
                                                        { x.MemberLastReadThreadMessageId}
                                                    </td>
                                                    <td>
                                                        {
                                                            x.MemberPermissions &&
                                                            JSON.stringify(x.MemberPermissions)
                                                        }
                                                    </td>
                                                    <td>
                                                        {x.IsLeave}
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                {/* <li>
                                                                    <a onClick={() => router.push("/chat/channel/edit/" + x.ChatChannelId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteChatChannel(x.ChatChannelId)}>Delete</a>
                                                                </li> */}
                                                            </ul>
                                                        </div>
                                                    </td>
...
export default MemberThreadListing