...
import { reactChannelMessageDelete, messageReactionListing } from "../api";
...
const ChatChannelMessageReactionListing = () => {
    ...
    const getMessageReactions = async () => {
        setLoading(true)
        setMessageReactions([]);
        let result = await messageReactionListing({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setMessageReactions(result.Data.reactions)
    ...
    const deleteMessageReaction = (id) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Reaction with Id '" + id + "' ?", "Delete", confirmDeleteMessageReaction, id)
    }
    ...
    const confirmDeleteMessageReaction = async (id) => {
        mdLoading.current.show("Deleting..")
        let msgId = id.split('_')[0];
        let emoId = id.split('_')[1];
        let reacterId = id.split('_')[2];
        let result = await reactChannelMessageDelete({
            messageId: msgId,
            emojiId: emoId,
            reactorId: reacterId
        })
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => deleteMessageReaction(`${x.MessageId}_${x.EmojiId}_${x.MemberId}`)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
...
export default ChatChannelMessageReactionListing