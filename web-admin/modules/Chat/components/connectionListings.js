...
import { connectionListing } from "../api";
...
const ConnectionListing = () => {
    ...
    const getConnections = async () => {
        setLoading(true)
        setConnections([]);
        let result = await connectionListing({
            pageSize: pageSize, 
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">CHAT - CONNECTIONS</h2>   
            </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`chatrooms_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        Member Id: {x.Member.MemberId}<br/>
                                                        Wallet Address: {x.Member.WalletAddress}<br/>
                                                        DisplayName: {x.Member.DisplayName}<br/>
                                                    </td>
