...
const AvatarItemCreate = () => {
    ...
    const onSubmit = async (data) => {
        ...
        let result = await avatarItemPost({
        ...
        console.log("create avatar item result", result)
        
        if(result.Success) {
            mdLoading.current.close()
            showSuccess("Avatar Item created successfully")
            router.push("/metaverse/avataritem")
        }            
        else
            showFailed("Avatar Item created failed with message: " + result.Message)
    }
        ...
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <h2 className="ml-3 text-sm font-bold">CREATE AVATAR ITEM</h2>                        
                </div>
                <div className="p-4 w-full">
                    ...
                    <br/>
                    <button className="mt-5 btn btn-primary btn-sm" type="submit">Create Avatar Item</button>
...
export default AvatarItemCreate