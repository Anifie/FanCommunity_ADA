...
import { ticketDelete, ticketListing } from "../api";
...
const TicketListing = () => {
    ...
    const getTickets = async () => {
        setLoading(true)
        setTickets([]);
        let result = await ticketListing({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            ...
            if(result.Data.tickets.length > 0 && result.Data.lastKey) {
    ...
    const deleteTicket = (ticketId) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Ticket with Id '" + ticketId + "' ?", "Delete", confirmDeleteTicket, ticketId)
    }
    ...
    const confirmDeleteTicket = async (ticketId) => {
        ...
        let result = await ticketDelete({ticketId: ticketId})
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">TICKET</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/event/ticket/create")}>+ CREATE NEW</button>
                    </div>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-left">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => router.push("/event/ticket/edit/" + x.TicketId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteTicket(x.TicketId)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
