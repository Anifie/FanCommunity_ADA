...
const EventListing = () => {
    ...
    const getEvents = async () => {
        ...
        if(result.Success) {
            ...
            if(result.Data.events.length > 0 && result.Data.lastKey) {
    ...
    const deactivateEvent = (eventId) => {
        mdConfirm.current.show("Confirm", "Confirm Deactivate Event with Id '" + eventId + "' ?", "Deactivate", confirmDeactivateEvent, eventId)
    }
    ...
    const confirmDeactivateEvent = async (eventId) => {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">EVENT</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/event/events/create")}>+ CREATE NEW</button>
                    </div>
                                        ...
                                        .map(
                                            (x, index) => (
                                                <tr key={`events_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        <img src={x.ImageURL} className="w-[50px]" />
                                                    </td>
                                                    <td>
                                                        {x.EventId}
                                                    </td>
                                                    ...
                                                    <td>
                                                        { x.TicketPriceCurrencyCode} {x.TicketPrice}
                                                    </td>
                                                    <td>
                                                        { x.EventType}
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-down">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                {/* <li>
                                                                    <a onClick={() => deactivateEvent(x.EventId)}>Deactivate</a>
                                                                </li> */}
                                                                <li>
                                                                    <a onClick={() => router.push("/event/events/edit/" + x.EventId)}>Edit</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
