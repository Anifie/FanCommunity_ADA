...
import { voucherGet, voucherPost, voucherPut, voucherDelete, voucherListing } from "../api";
...
const VoucherListing = () => {
    ...
    const getVouchers = async () => {
        setLoading(true)
        setVouchers([]);
        let result = await voucherListing({
            pageSize: pageSize, 
            lastKey: pages[pageIndex], 
            isAvailable: status == 'AVAILABLE' ? true : (status == 'TAKEN' ? false : undefined), 
            eventId: eventId
        })
        ...
        if(result.Success) {
            ...
            if(result.Data.vouchers.length > 0 && result.Data.lastKey) {
    ...
    const deleteVoucher = (voucherId) => {
        mdConfirm.current.show("Confirm", "Confirm Delete Voucher with Id '" + voucherId + "' ?", "Delete", confirmDeleteVoucher, voucherId)
    }
    ...
    const confirmDeleteVoucher = async (voucherId) => {
        ...
        let result = await voucherDelete({voucherId : voucherId})
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">VOUCHER</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/event/voucher/create")}>+ CREATE NEW</button>
                    </div>
                        ...
                        <tbody>
                            {
                                ...
                                vouchers
                                        .map(
                                            (x, index) => (
                                                <tr key={`events_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="dropdown dropdown-down">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    <a onClick={() => router.push("/event/voucher/edit/" + x.VoucherId)}>Edit</a>
                                                                </li>
                                                                <li>
                                                                    <a onClick={() => deleteVoucher(x.VoucherId)}>Delete</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
