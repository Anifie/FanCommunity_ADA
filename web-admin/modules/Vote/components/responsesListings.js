...
const MemberResponseListing = () => {

    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [voteReports, setVoteReports] = useState([])
    ...
    const getVoteReport = async () => {
        setLoading(true)
        setVoteReports([]);
        let result = await voteReport({
            // pageSize: pageSize, 
        ...
        console.log("vote report result", result);
        if(result.Success) {
            setVoteReports(result.Data.votes)
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">Vote Report</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    ...
                    <table className={`table table-compact w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#242933]'}`}>
                        ...
                        <tbody>
                            {
                                voteReports && voteReports.length > 0
                                ?
                                voteReports
                                        .map(
                                            (x, index) => (
                                                <tr key={`report_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    ...
                                                    <td>
                                                        {x.TokenIds ? `Contract Address : ${x.ContractAddress} , TokenId : ${x.TokenId}` : ''}
                                                    </td>
                                                    <td>
                                                        {x.ArtworkId ? x.ArtworkId : ''}
                                                    </td>
                                                    <td>
...
export default MemberResponseListing