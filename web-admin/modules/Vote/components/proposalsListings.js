...
import { proposalListingPost, proposalPost, proposalDelete, proposalPut, voteReport } from "../api";
...
const ProposalListing = () => {
    ...
    const getProposals = async () => {
        setLoading(true)
        setProposals([]);
        let result = await proposalListingPost({
            // pageSize: pageSize, 
    ...
    const deleteProposal = (proposalId) => {
        mdConfirm.current.show("Confirm", "Confirm delete proposal and its member vote ?", "Delete Proposal", confirmDeleteProposal, proposalId)
    }

    const confirmDeleteProposal = async (proposalId) => {
        mdLoading.current.show("Deleting proposal..")
        
        let result = await proposalDelete({
            voteProposalId: proposalId
        })
        console.log("delete proposal result", result);
        if(result?.Success){
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">Vote Proposal</h2>   
            </div>
            <div className="p-4 w-full">
                <div className={`relative flex flex-col break-words rounded-sm min-w-0 items-center ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                    <div className="w-full justify-start">
                        <button className="btn btn-primary btn-sm my-2 ml-2" onClick={() => router.push("/vote/proposals/new")}>+ NEW VOTE PROPOSAL</button>
                    </div>
                        ...
                        <tbody>
                            {
                                ...
                                proposals
                                        .map(
                                            (x, index) => (
                                                <tr key={`proposal_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    {/* <td>
                                                        <Checkbox
                                                            key={x.AssetId}
                                                            type="checkbox"
                                                        ...
                                                        <div className="dropdown dropdown-down">
                                                            ...
                                                            <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-[#212529] rounded-box w-52">
                                                                <li>
                                                                    ...
                                                                    <a onClick={() => router.push("/vote/proposals/edit/" + x.VoteProposalId)}>Edit</a>
                                                                    <a onClick={() => deleteProposal(x.VoteProposalId)}>Delete</a>
