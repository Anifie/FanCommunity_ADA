...
import { invitationListingPost } from "../api";
...
const AnnouncementListing = () => {
    ...
    const getAnnouncements = async () => {
        setLoading(true)
        setAnnoucements([]);
        let result = await invitationListingPost({
            pageSize: pageSize, 
        ...
        if(result.Success) {
            setAnnoucements(result.Data.invitations)
            setLastPageIndex(null);
            if(result.Data.invitations.length > 0 && result.Data.nextToken) {
    ...
    return (
        <div className="flex flex-col justify-start items-start h-full w-full text-slate-100">
            ...
            <div className={`h-12 flex items-center w-full ${process.env.IS_TEST == 'true' ? 'bg-[#00224B]' : 'bg-[#212529]'}`}>
                <h2 className="ml-3 text-sm font-bold">INVITATIONS</h2>   
            </div>
                        ...
                        <tbody>
                            {
                                ...
                                annoucements
                                        .map(
                                            (x, index) => (
                                                <tr key={`invitation_` + index} className={`${process.env.IS_TEST == 'true' ? 'bg-[#00316E]' : 'bg-[#2a303c]'}`}>
                                                    <td>
                                                        {index + 1}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        { x.AcceptedDate && moment(x.AcceptedDate).format('YYYY-MM-DD')}
                                                        <br />
                                                        { x.AcceptedDate && moment(x.AcceptedDate).format('HH:mm')}
                                                    </td>
                                                    {/* <td>
                                                        {x.Role}
                                                    </td>
