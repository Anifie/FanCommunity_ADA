...
function ToDiscordMessage(obj) {
    ...
    */
    ...
    let msg = {
        ...
        Message: obj.content,
        ThreadId: obj.discord_thread_id,
        Attachments: obj.raw_attachments ? JSON.parse(obj.raw_attachments) : []
    };
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (body.channelId) {
        sql += ` and discord_channel_id = '${body.channelId}' `;
    }
    if (body.threadId) {
        sql += ` and discord_thread_id = '${body.threadId}' `;
    }
    ...
    return {
      Success: true,
      Data: {
        messages: allMessages.map(x => ToDiscordMessage(x)),
        nextToken: nextToken
      }
    }    
    ...
    console.error('error in ada-analytic-chat-message-listing-post ' + random10DigitNumber, e);

    const message = {
        Subject: 'Error - ada-analytic-chat-message-listing-post- ' + random10DigitNumber,
        Message: `Error in ada-analytic-chat-message-listing-post ${e.message}\n\nStack trace:\n${e.stack}`,
