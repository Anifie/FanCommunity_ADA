...
export const handler = async (event) => {
    ...
    try {
        ...
        sql = `update "${tableName}" set enum_values = 'DONE' where type = 'ENUM' and PK = 'ENUM' and SK = '${body.queueType}' `;
        console.log("sql", sql);
        ...
        console.error('error in ada-nft-queue-status-reset-post ' + random10DigitNumber, e);
        
        const message = {
            Subject: 'ADA Error - ada-nft-queue-status-reset-post - ' + random10DigitNumber,
            Message: `Error in ada-nft-queue-reset-status-post: ${e.message}\n\nStack trace:\n${e.stack}`,
