...
export const handler = async (event) => {
    ...
    try {
        ...
        if(body.minAmount) {
            sql += `, amount_min = ${body.minAmount} `;
        }
        if(body.maxAmount) {
            sql += `, amount_max = ${body.maxAmount} `;
        }
        if(body.color) {
            sql += `, color = '${body.color}' `;
        }
        if(body.status) {
            sql += `, status = '${body.status}' `;
        }
        if(body.durationInMinutes) {
            sql += `, duration_in_minutes = ${body.durationInMinutes} `;
        }

        sql += ` where PK = '${superChatTemplate.PK}' and SK = '${superChatTemplate.SK}'`;
        ...
        console.error('error in ada-chat-super-template-put ' + random10DigitNumber, e);
    
        const message = {
            Subject: 'ADA Error - ada-chat-super-template-put- ' + random10DigitNumber,
            Message: `Error in ada-chat-super-template-put ${e.message}\n\nStack trace:\n${e.stack}`,
